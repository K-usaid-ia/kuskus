...
export default function ProjectStats({ projects = mockProjects }: ProjectStatsProps) {
  // Calculate statistics
  const stats = {
    totalProjects: projects.length,
    activeProjects: projects.filter(p => p.status === 'active').length,
    totalBudget: projects.reduce((sum, p) => sum + p.budget, 0),
    projectsWithMilestones: projects.filter(p => p.milestones.length > 0).length
  };

  // Calculate progress metrics
  const completedMilestones = projects.reduce(
    (sum, project) => sum + project.milestones.filter(m => m.status === 'completed').length,
    0
  );
